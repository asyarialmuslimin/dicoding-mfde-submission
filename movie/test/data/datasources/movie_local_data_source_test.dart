...
void main() {
  ...
  setUp(() {
    ...
    dataSource = MovieLocalDataSourceImpl(movieDao: mockMovieDao);
  });
  ...
  group('save watchlist', () {
    test(
      ...
      () async {
        // arrange
        when(
          mockMovieDao.insertWatchlist(testMovieTable),
  ...
  group('remove watchlist', () {
    test(
      ...
      () async {
        // arrange
        when(
          mockMovieDao.removeWatchlist(testMovieTable),
        ).thenAnswer((_) async => 1);
        // act
  ...
  group('Get Movie Detail By Id', () {
    ...
    test('should return null when data is not found', () async {
      // arrange
      when(mockMovieDao.getMovieById(tId)).thenAnswer((_) async => null);
      // act
      final result = await dataSource.getMovieById(tId);
      // assert
      expect(result, null);
  ...
  group('get watchlist movies', () {
    test('should return list of MovieTable from database', () async {
      // arrange
      when(
        mockMovieDao.getWatchlistMovies(),
      ).thenAnswer((_) async => [testMovieMap]);
      // act
